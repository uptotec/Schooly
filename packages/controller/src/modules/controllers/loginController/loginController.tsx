...
interface props {
  children: (data: LoginControllerData) => JSX.Element | null;
}
...
export const LoginController: React.FunctionComponent<props> = (props) => {
  ...
  ] = useLazyQuery<studentLoginData, LoginType>(STUDENT_LOGIN_REQUEST, {
    fetchPolicy: 'no-cache',
  });
  const [staffLogin, { data: staffData, loading: staffLoading }] = useLazyQuery<
    staffLoginData,
    LoginType
  >(STAFF_LOGIN_REQUEST, { fetchPolicy: 'no-cache' });
...
const STUDENT_LOGIN_REQUEST = gql`
  query($email: String!, $password: String!) {
    studentLogin(credentials: { email: $email, password: $password }) {
      studentId
      name
      email
      facility {
...
const STAFF_LOGIN_REQUEST = gql`
  query($email: String!, $password: String!) {
    staffLogin(credentials: { email: $email, password: $password }) {
      staffId
      name
      email
      facility {
